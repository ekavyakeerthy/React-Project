...
const Footer = () => {
  const [footerData, setFooterData] = useState({
    companyName: "Super Prime",
  description: "Glow every day with our cured skincare and makeup essentials.",
    copyright: "© 2021-2022",
    links: [
      { name: "Contact", path: "/contact" },
      { name: "TermsandConditions", path: "/terms-and-conditions" },
    ]
  });
  ...
  const handleFooterLinkClick = async (linkName) => {
    try {
  ...
  const handleLogoClick = async () => {
    try {
      await fetch('/api/track-interaction', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'footer_logo_click' })
      });
    } catch (error) {
      console.error('Failed to track logo click:', error);
    }
  };
    ...
    <>
      <footer className="p-4 bg-white md:p-8 lg:p-10 dark:bg-gray-800">
        <div className="mx-auto max-w-screen-xl text-center">
          <Link
            to="/"
            className="flex justify-center items-center text-2xl font-semibold text-gray-900 dark:text-white hover:text-primary-700 dark:hover:text-primary-400 transition-colors duration-200"
          ...
          >
            ...
            {isLoading ? 'Loading...' : footerData.companyName}
          </Link>

          <p className="my-6 text-gray-500 dark:text-gray-400">
            {isLoading ? 'Loading description...' : footerData.description}
          </p>


          {/* Newsletter Subscription */}
          <div className="mb-6">
            <form onSubmit={handleNewsletterSubscribe} className="flex flex-col sm:flex-row gap-2 justify-center items-center">
              <input
                type="email"
              ...
              <button
                type="submit"
                disabled={isSubscribing}
                className="px-6 py-2 bg-primary-700 text-white rounded-lg hover:bg-primary-800 focus:ring-2 focus:ring-primary-500 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubscribing ? 'Subscribing...' : 'Subscribe'}
              </button>
            </form>
            {subscriptionMessage && (
              <p className={`mt-2 text-sm ${subscriptionMessage.includes('Thank you') ? 'text-green-600' : 'text-red-600'}`}>
                {subscriptionMessage}
              </p>
            )}
          </div>
          ...
          <span className="text-sm text-gray-500 sm:text-center dark:text-gray-400">
            ...
            <Link to="/" className="hover:underline hover:text-primary-700 dark:hover:text-primary-400 transition-colors duration-200" onClick={handleLogoClick}>
